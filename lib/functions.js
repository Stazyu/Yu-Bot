...
const { writeFile, readFile, readFileSync, writeFileSync, fstat } = require('fs');
...
const downloadMediaWa = async (msg, ext = '', nameFile = '') => {
	const { chatMessage, isQuotedMedia } = msg;
	const mediaMessage = isQuotedMedia ? JSON.parse(JSON.stringify(chatMessage).replace('quotedM', 'm')).extendedTextMessage.contextInfo.message : chatMessage;
...
const sendText = async (from, text, options = {}) => {
	const msg_result = await wa.sendMessage(from, { text, mentions: options.mentions });
	return msg_result;
}
...
const reply = async (from, text, msg, options = {}) => {
	const msg_result = await wa.sendMessage(from, { text: text, mentions: options.mentions }, { quoted: msg });
	return msg_result;
}
 ...
 * @returns 
 */
const sendSticker = async (from, buff, options = {}) => {
	if (typeof buff === 'string') {
		const msg_result = await wa.sendMessage(from, { sticker: { url: buff } } || { height: options.height, width: options.width, mentions: options.mentions }, { quoted: options.msg });
		return msg_result;
	} else {
		const msg_result = await wa.sendMessage(from, { sticker: buff } || { height: options.height, width: options.width, mentions: options.mentions }, { quoted: options.msg });
		return msg_result;
	}
}
...
module.exports = { connection, downloadMediaWa, sendText, reply, sendAudio, sendImage, sendVideo, sendDocument, sendSticker, sendStickerAsImage, sendMedia, sendButton, sendButtonWithImage, getBuffer, sendTemplateButton, sendTemplateButtonWithImage, sendListMessage, sendListMessageWithImage };
