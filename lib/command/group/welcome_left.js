...
➾ prefix.lefttext [Untuk mengatur text sapaan member keluar]
    ...
    async execute(msg, conn) {
        const { args, chat, command, fromMe, groupId, groupName, from, isGroupAdmins, isOwner, prefix } = msg;
        switch (command) {
            case 'welcome':
                if (!fromMe && !isGroupAdmins && !isOwner) return bot.reply(from, 'Mohon Maaf.. Fitur ini khusus Admin Grup!', chat);
