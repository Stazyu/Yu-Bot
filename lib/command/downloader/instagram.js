...
`
            for (let i = 0; i < ig.result.medias.length; i++) {
                if (i === 0) {
                    await sendMedia(from, ig.result.medias[i].url, { caption: capt, quoted: chat });
                } else {
                    await sleep(3000);
                    await sendMedia(from, ig.result.medias[i].url);
                }
            }
        } catch {
            try {
                ...
                for (let i = 0; i < ig.link.length; i++) {
                    if (i == 0) {
                        console.log(ig.link);
                        await sendMedia(from, ig.link[i], { caption: 'Nih kak..' });
                    } else {
                        await sleep(3000);
                        await sendMedia(from, ig.link[i]);
                    }
                }
            } catch (err) {
                console.log(err);
