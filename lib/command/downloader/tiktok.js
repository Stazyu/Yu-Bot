...
Semoga Bermanfaat..`
                const link = far;
                const tkt = await tiktok(link);
                console.log(tkt);
                const capt = `*『 Tiktok Downloader 』*\n
*➨ Caption :* ${tkt.result.title} `

                addMediaSession({
                    user_id,
                    urlWm: tkt.result.medias[0].url,
                    urlNowm: tkt.result.medias[1].url,
                    urlMusic: tkt.result.medias[2].url,
                    type: 'tkt'
                })
                ...
                await sendButtonWithImage(from, buttons, tkt.result.thumbnail, { caption: capt, footer: capt_footer, viewOnce: false });
            } catch (error) {
                console.log(error);
