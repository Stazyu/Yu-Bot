...
➾ ${prefix}welcome <on atau off>
➾ ${prefix}welcometext <set atau default>
➾ ${prefix}left <on atau off> 
➾ ${prefix}lefttext <set atau default>
➾ ${prefix}afk <reason>
...
const infoBot = async (msg) => {
    const { runtime, time } = msg;
    const allUser = (await db.findAllUser()).length;
    const dateNow = await tanggal();
    try {
        return `Info Bot
>> Nama Bot : Yu-Bot
>> Versi Bot : 1.0.0 (BETA)
>> Total user Bot : ${allUser}
>> Runtime : ${runtime}
...
➾ ${prefix}welcome [Sapaan saat ada member masuk]
...
➾ ${prefix}afk [Afk User]
    - Format : ${prefix}afk <reason>
    - Contoh : ${prefix}afk Main Game
...
const mess = {
    ...
    info: {
        onlyGroup: 'Maaf, fitur hanya bisa digunakan di dalam Group!'
    },
    menu: {
...
module.exports = { menu, mess, help, infoBot }